/**
 * @module SupplierApiTraceAudit
 * @ownership PlatformGovernance
 * @description Correlates supplier UI API calls with NestJS controllers to prevent drift in payload contracts and HTTP verbs.
 */
...
interface SupplierApiTraceReport {
  generatedAt: string;
...
const SUPPLIER_FILE_PATTERNS = [
  'apps/web/app/(supplier)/**/*.{ts,tsx}',
  'apps/web/components/**/supplier/**/*.{ts,tsx}',
  'apps/web/components/(supplier)/**/*.{ts,tsx}',
  'apps/web/lib/**/supplier/**/*.{ts,tsx}',
];

const FETCH_ALIASES = new Set(['fetch']);
const AXIOS_ALIASES = new Set(['axios', 'apiClient', 'supplierClient']);
...
function collectSupplierApiCalls(): ApiCallDescriptor[] {
  const results: ApiCallDescriptor[] = [];
  const supplierFiles = globby.sync(SUPPLIER_FILE_PATTERNS, { cwd: repoRoot });
  ...
  for (const file of supplierFiles) {
    ...
    const route = file.startsWith('apps/web/app') ? normalizeSupplierRoute(file) : 'shared-component';
    ...
    for (const call of callExpressions) {
      ...
      if (Node.isIdentifier(expression) && FETCH_ALIASES.has(expression.getText())) {
        ...
        if (!url || (!url.startsWith('/api') && !url.includes('/supplier'))) {
          continue;
        }
...
async function main(): Promise<void> {
  const calls = collectSupplierApiCalls();
  ...
  const report: SupplierApiTraceReport = {
    generatedAt: new Date().toISOString(),
    calls: traceEntries,
  };

  await mkdir(outputDir, { recursive: true });
  const jsonPath = path.join(outputDir, 'supplier-api-trace.json');
  await writeFile(jsonPath, `${JSON.stringify(report, null, 2)}\n`, 'utf-8');

  const markdown: string[] = [
    '# Supplier API Trace Audit',
    '',
  ...
  const markdownPath = path.join(outputDir, 'supplier-api-trace.md');
  await writeFile(markdownPath, `${markdown.join('\n')}\n`, 'utf-8');

  // eslint-disable-next-line no-console
  console.log(`Supplier API trace audit written to ${jsonPath} and ${markdownPath}`);
}
