-- ============================================================================
-- COMBINED MIGRATION 0039 FOR SUPABASE SQL EDITOR
-- Step 12: Capacity Ledger + Lead Time Classes
-- ============================================================================
-- Execute this entire file in Supabase SQL Editor in one operation
-- ============================================================================
...
-- Indexes for efficient capacity window queries
...
--
-- SELECT 
--   (SELECT COUNT(*) FROM leadtime_profiles) as profiles_count,
--   (SELECT COUNT(DISTINCT org_id) FROM leadtime_profiles) as orgs_with_profiles,
--   (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'capacity_ledger') as capacity_table_exists,
--   (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'leadtime_overrides') as overrides_table_exists;
--
-- Expected: profiles_count = (number of orgs × 4 processes), capacity_table_exists = 1, overrides_table_exists = 1
-- ============================================================================
