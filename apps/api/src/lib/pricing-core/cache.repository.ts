...
export class PricingCacheRepository {
  ...
  async recordHit(id: number, params: { ttlAt?: string } = {}): Promise<void> {
    ...
    const { error } = await this.supabase.client
      .rpc("pricing_cache_record_hit", { cache_id: id, ttl_at_param: params.ttlAt ?? null, now_param: nowIso });

    if (error) {
      const update: Record<string, unknown> = { last_hit_at: nowIso };
      if (params.ttlAt) {
        update.ttl_at = params.ttlAt;
      }

      await this.supabase.client.from("pricing_cache").update(update).eq("id", id);
    }
  }
