...
export default function GuestInstantQuoteFlow() {
  ...
  const uploadAborters = useRef<Record<string, AbortController>>({});

  const hasAtLeastOneFile = files.some((f) => f.status === 'pending' || f.status === 'uploading' || f.status === 'uploaded');
  ...
  const handleFilesSelected = useCallback((incoming: File[]) => {
    ...
    incoming.forEach((file) => {
      ...
      next.push({ id, file, status: err ? 'error' : 'pending', progress: 0, error: err ? getErrorMessage(err) : undefined });
    });
  ...
  const presignAndUpload = useCallback(async (entry: SelectedGuestFile): Promise<SelectedGuestFile> => {
    try {
      // Presign
      ...
      const tick = setInterval(() => {
        setFiles((prev) => prev.map((f) => (f.id === entry.id && f.progress < 95 ? { ...f, progress: f.progress + 5 } : f)));
      }, 200);
      ...
      try {
        if (uploadMethod === 'POST') {
          ...
          const up = await fetch(presign.url, { method: 'POST', body: formData, credentials: 'omit' });
          if (!up.ok) throw new Error(`Upload failed: ${up.status}`);
        } else {
          const headers = presign.headers ? new Headers(presign.headers) : new Headers();
          if (!headers.has('Content-Type') && entry.file.type) headers.set('Content-Type', entry.file.type);
          const up = await fetch(presign.url, { method: 'PUT', body: entry.file, headers, credentials: 'omit' });
          if (!up.ok) throw new Error(`Upload failed: ${up.status}`);
        }
      } finally {
        clearInterval(tick);
      ...
      return { ...entry, status: 'error', error: getErrorMessage('UPLOAD_FAILED') };
    }
  }, []);

  const onSignupComplete = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      for (const f of toUpload) {
        ...
        setFiles((prev) => prev.map((p) => (p.id === result.id ? result : p)));
      }
      ...
      const quoteRes = await fetch('/api/quotes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ source: 'web', guestEmail: email, files: filePayload }),
      ...
      setFiles((prev) => prev.map((f) => (f.status === 'uploading' ? { ...f, status: 'error', error: err instanceof Error ? err.message : 'Failed. Try again.' } : f)));
    } finally {
