...
interface InternalQuoteContextType {
  ...
  updatePricingBreakdown: (lineId: string, breakdown: Partial<PricingBreakdown>) => Promise<void>
...
const InternalQuoteContext = createContext<InternalQuoteContextType | undefined>(undefined)
...
export function InternalQuoteProvider({ children, quoteId }: InternalQuoteProviderProps) {
  ...
  const fetchQuote = async () => {
    try {
      ...
      const response = await fetch(`/api/quotes/${quoteId}?include=lines,pricing,dfm,activity`)
      if (!response.ok) {
  ...
  const updatePricingBreakdown = async (lineId: string, breakdown: Partial<PricingBreakdown>) => {
    try {
      const response = await fetch(`/api/admin/quotes/${quoteId}/lines/${lineId}/override`, {
        method: 'PUT',
        headers: {
...
export function useInternalQuote() {
  ...
  if (context === undefined) {
    throw new Error('useInternalQuote must be used within an InternalQuoteProvider')
  }
  return context
}
