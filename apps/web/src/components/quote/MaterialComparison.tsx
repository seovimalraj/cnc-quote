...
export function MaterialComparison({
  ...
  const [filter, setFilter] = useState<'all' | 'cost' | 'strength' | 'machinability'>('all');
  const [sortBy, setSortBy] = useState<'score' | 'cost' | 'strength' | 'machinability'>('score');
  ...
  }, [materialId, processType, geometry, features, quantity, machineId, region]);
  ...
  const filteredAlternatives = comparison?.alternatives.filter((alt) => {
    if (filter === 'all') return true;
    if (filter === 'cost') return alt.comparison.cost_delta_pct < 0; // Cheaper
    if (filter === 'strength') return (alt.properties.tensile_mpa || 0) > (comparison.current.properties.tensile_mpa || 0);
    if (filter === 'machinability') return alt.properties.machinability_index > comparison.current.properties.machinability_index;
    return true;
  }) || [];
  ...
  const sortedAlternatives = [...filteredAlternatives].sort((a, b) => {
    ...
    if (sortBy === 'strength') return (b.properties.tensile_mpa || 0) - (a.properties.tensile_mpa || 0);
    if (sortBy === 'machinability') return b.properties.machinability_index - a.properties.machinability_index;
    return 0;
  });
          ...
          <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <Select value={filter} onValueChange={(value: any) => setFilter(value)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="machinability">More Machinable</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <Select value={sortBy} onValueChange={(value: any) => setSortBy(value)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="machinability">Most Machinable</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-gray-500">
                Best value: {comparison.alternatives.find((a) => a.is_best_value)?.material.name}
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardTitle className="mt-2">{comparison.current.material.name}</CardTitle>
              <p className="text-sm text-gray-600">{comparison.current.material.code}</p>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold">{formatCurrency(comparison.current.pricing.unit_price)}</div>
        ...
        <CardContent>
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <div className="font-semibold">{comparison.current.properties.density_kg_m3} kg/m³</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Machinability</div>
              <div className="font-semibold">{comparison.current.properties.machinability_index}/100</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Tensile Strength</div>
              <div className="font-semibold">
                {comparison.current.properties.tensile_mpa ? `${comparison.current.properties.tensile_mpa} MPa` : 'N/A'}
              </div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Lead Time</div>
              <div className="font-semibold">{comparison.current.pricing.lead_time_days} days</div>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          >
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-xl font-bold">{formatCurrency(alt.pricing.unit_price)}</div>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  <div className="font-semibold">{alt.properties.machinability_index}/100</div>
                </div>
                <div>
                  <div className="text-xs text-gray-600">Strength</div>
                  <div className="font-semibold">
                    {alt.properties.tensile_mpa ? `${alt.properties.tensile_mpa} MPa` : 'N/A'}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-600">Weight</div>
                  <div className="font-semibold">{alt.comparison.weight_g.toFixed(0)}g</div>
                </div>
                <div>
                  <div className="text-xs text-gray-600">Lead Time</div>
                  <div className="font-semibold">{alt.pricing.lead_time_days} days</div>
                </div>
              </div>
              ...
              {alt.compatibility.warnings.length > 0 && (
                <div className="mb-4 p-2 bg-yellow-50 border border-yellow-200 rounded">
                  {alt.compatibility.warnings.map((warning, i) => (
                    <div key={i} className="text-xs text-yellow-800 flex items-center">
              ...
              >
                {alt.is_best_value && <CheckCircleIcon className="h-4 w-4 mr-2" />}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-3 bg-green-50 rounded-lg">
                ...
                <div className="font-semibold text-green-700">
                  {comparison.alternatives.find((a) => a.material.id === comparison.metadata.best_value_id)?.material.name}
                </div>
              </div>
              <div className="p-3 bg-blue-50 rounded-lg">
                <div className="text-sm text-gray-600">Cheapest</div>
                <div className="font-semibold text-blue-700">
                  {comparison.alternatives.find((a) => a.material.id === comparison.metadata.cheapest_id)?.material.name}
                </div>
              </div>
              <div className="p-3 bg-purple-50 rounded-lg">
                <div className="text-sm text-gray-600">Strongest</div>
                <div className="font-semibold text-purple-700">
                  {comparison.alternatives.find((a) => a.material.id === comparison.metadata.strongest_id)?.material.name}
                </div>
              </div>
              <div className="p-3 bg-orange-50 rounded-lg">
                <div className="text-sm text-gray-600">Most Machinable</div>
                <div className="font-semibold text-orange-700">
                  {comparison.alternatives.find((a) => a.material.id === comparison.metadata.most_machinable_id)?.material.name}
                </div>
              </div>
            </div>
          </CardContent>
