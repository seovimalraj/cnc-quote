/**
 * @module api/admin/metrics/slo
 * @ownership platform-ops
 * @description Proxies admin SLO snapshot requests to the API and validates against shared contracts.
 */
...
export async function GET(request: NextRequest) {
  ...
  const target = new URL(resolveApiUrl('/admin/metrics/slo'));
  if (window) {
    target.searchParams.set('window', window);
  }

  const upstream = await proxyFetch(request, target, { method: 'GET' });
  return forwardJsonWithSchema(upstream, ContractsVNext.AdminSloSnapshotSchema);
}
