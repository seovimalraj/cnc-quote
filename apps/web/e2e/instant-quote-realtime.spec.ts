...
test.describe('Instant Quote Realtime Flow', () => {
  test('uploads two parts and observes pricing subtotal progression', async ({ page }) => {
    ...
    const initialSubtotalText = await page.getByTestId('subtotal').textContent();
    ...
    for (let i = 0; i < 40; i++) { // 40 * 500ms = 20s
      ...
      if (val && val > 0) { finalSubtotal = val; break; }
      await page.waitForTimeout(500);
    }

    // Not all environments will emit pricing; soft assert if absent
    if (!finalSubtotal) {
      test.info().annotations.push({ type: 'note', description: 'Subtotal did not update > 0; backend pricing events may be disabled in this environment.' });
    } else {
