...
const cloneHeaders = (source: HeaderSource): Headers => {
  ...
  const candidate = source as { forEach?: (cb: (value: string, key: string) => void) => void };
...
export async function proxyFetch(
  ...
  const method = (init.method ?? (req as Request | undefined)?.method ?? 'GET').toUpperCase();
...
export async function forwardJsonWithSchema<T>(upstream: Response, schema: z.ZodSchema<T>) {
  if (!upstream.ok) {
