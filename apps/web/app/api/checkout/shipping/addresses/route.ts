...
export async function GET(request: NextRequest) {
  try {
    ...
    const defaultOnly = request.nextUrl.searchParams.get('defaultOnly') === 'true';
    ...
    return NextResponse.json({ addresses: (data ?? []).map(mapRecordToClient) });
  } catch (error) {
    if (error instanceof MissingOrgContextError) {
      return NextResponse.json({ error: error.message }, { status: 401 });
    }

    const message = error instanceof Error ? error.message : 'Failed to load shipping addresses';
    console.error('Shipping address load failed:', message);
    return NextResponse.json({ error: 'Failed to load shipping addresses' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Shipping address save failed:', message);
