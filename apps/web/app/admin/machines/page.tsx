...
export default function AdminMachinesPage() {
  ...
  const getStatusBadge = (status: Machine['status']) => {
    const variants = {
      operational: { bg: 'bg-green-500/10 text-green-500 border-green-500/20', icon: CheckCircle },
      maintenance: { bg: 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20', icon: Wrench },
      offline: { bg: 'bg-red-500/10 text-red-500 border-red-500/20', icon: XCircle },
      idle: { bg: 'bg-blue-500/10 text-blue-500 border-blue-500/20', icon: Activity }
    };
  ...
  const stats = {
    ...
    avgUtilization: Math.round(
      mockMachines.reduce((sum, m) => sum + m.utilization, 0) / mockMachines.length
    )
  };
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Monitor and manage manufacturing equipment</p>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-white mt-1">{stats.operational}</p>
            </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-white mt-1">{stats.avgUtilization}%</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredMachines.map((machine) => (
          <Card key={machine.id} className="bg-slate-800/50 border-slate-700 p-6 hover:bg-slate-800/70 transition-colors">
            {/* Header */}
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">{machine.supplier}</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-900/50 rounded-lg p-3">
                ...
                <p className="text-sm font-medium text-white">{machine.specifications.axes}</p>
              </div>
              <div className="bg-slate-900/50 rounded-lg p-3">
                <p className="text-xs text-gray-400 mb-1">Work Envelope</p>
                <p className="text-sm font-medium text-white">{machine.specifications.workEnvelope}</p>
              </div>
              <div className="bg-slate-900/50 rounded-lg p-3">
                <p className="text-xs text-gray-400 mb-1">Spindle Speed</p>
                <p className="text-sm font-medium text-white">{machine.specifications.spindle}</p>
              </div>
              <div className="bg-slate-900/50 rounded-lg p-3">
                <p className="text-xs text-gray-400 mb-1">Tool Capacity</p>
                <p className="text-sm font-medium text-white">
                  {machine.specifications.toolCapacity > 0 ? `${machine.specifications.toolCapacity} tools` : 'N/A'}
                </p>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-sm font-semibold ${getUtilizationColor(machine.utilization)}`}>
                  {machine.utilization}%
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-white">{machine.performance}%</span>
            ...
            <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-white">{machine.nextMaintenance}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-400">Hours Run</p>
                <p className="text-sm font-medium text-white">{machine.hoursRun.toLocaleString()}h</p>
              </div>
            </div>
