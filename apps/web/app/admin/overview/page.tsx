...
import { 
  Plus, Search, Eye, Edit, Trash2, MessageSquare, Package, Factory,
  User, DollarSign, TrendingUp, Clock, CheckCircle, AlertCircle, Settings
} from 'lucide-react';
import { 
  getOrders, getCustomers, getSuppliers, getMachines, getParts, getMessages,
  createOrder, updateOrder, deleteOrder, assignSupplierToOrder,
  createCustomer, updateCustomer, deleteCustomer,
  getUnreadMessageCount, markMessageAsRead,
  Order, Customer, Supplier, Machine, Part, Message
...
export default function AdminOverviewPage() {
  ...
  const handleCreateOrder = () => {
    ...
    setNewOrder({ customer_id: '', notes: '', shipping_address: '', priority: 'medium', due_date: '' });
  };
  ...
  const stats = {
    ...
    activeOrders: orders.filter(o => ['quoted', 'approved', 'in_production', 'quality_check'].includes(o.status)).length,
    ...
    avgMachineUtilization: machines.length > 0 
      ? Math.round(machines.reduce((sum, m) => sum + m.current_utilization, 0) / machines.length)
      : 0,
    unreadMessages: messages.filter(m => m.recipient_role === 'admin' && !m.read).length
  };

  const recentOrders = orders.slice(0, 10).sort((a, b) => 
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowCreateOrderDialog(true)} className="bg-blue-600 hover:bg-blue-700">
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-green-600 mt-1">{stats.activeOrders} active</p>
            </div>
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">${(stats.totalRevenue / 1000).toFixed(0)}K</p>
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-green-600 mt-1">{stats.activeCustomers} active</p>
            </div>
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{stats.totalMachines}</p>
              <p className="text-sm text-green-600 mt-1">{stats.operationalMachines} operational</p>
            </div>
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{stats.avgMachineUtilization}%</p>
      ...
      {urgentOrders.length > 0 && (
        <Card className="bg-red-50 border-red-200 shadow-sm p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-red-900 mb-2">Urgent Orders Requiring Attention</h3>
                ...
                {urgentOrders.map(order => (
                  <div key={order.id} className="flex items-center justify-between bg-white rounded-lg p-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{order.status.replace(/_/g, ' ')}</p>
                    </div>
      ...
      <Tabs defaultValue="orders" className="space-y-6">
              ...
              {recentOrders.map((order) => (
                <div key={order.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{order.customer_name} • {order.parts_count} parts</p>
                    <p className="text-sm text-gray-500">Updated {new Date(order.updated_at).toLocaleDateString()}</p>
                  </div>
                  <div className="text-right mr-4">
                    <p className="text-xl font-bold text-gray-900">${order.total_value.toLocaleString()}</p>
            ...
            <div className="space-y-3">
              {customers.map((customer) => (
                <div key={customer.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="text-right mr-4">
                    <p className="text-lg font-bold text-gray-900">{customer.total_orders} orders</p>
                    <p className="text-sm text-green-600">${(customer.total_spent / 1000).toFixed(0)}K spent</p>
                  </div>
                ...
                <div key={supplier.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {supplier.completed_orders}/{supplier.total_orders} completed • 
                  ...
                  <div className="text-right mr-4">
                    ...
                    <p className={`text-lg font-bold ${
                      supplier.capacity_utilization >= 90 ? 'text-red-600' :
                      supplier.capacity_utilization >= 75 ? 'text-yellow-600' :
                      'text-green-600'
                    }`}>
        ...
        <TabsContent value="machines" className="space-y-4">
          <Card className="bg-white border-gray-200 shadow-sm p-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">All Machines</h2>
              <p className="text-sm text-gray-600">
                {stats.operationalMachines} operational • Avg {stats.avgMachineUtilization}% utilized
              </p>
            </div>
            <div className="space-y-3">
              {machines.map((machine) => {
                const supplier = suppliers.find(s => s.id === machine.supplier_id);
                return (
                  <div key={machine.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        <h3 className="font-semibold text-gray-900">{machine.name}</h3>
                        <Badge className={`text-xs ${
                          machine.status === 'operational' ? 'bg-green-500/10 text-green-600' :
                          machine.status === 'maintenance' ? 'bg-yellow-500/10 text-yellow-600' :
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-lg font-bold ${
                        machine.current_utilization >= 90 ? 'text-red-600' :
                        machine.current_utilization >= 75 ? 'text-yellow-600' :
                        'text-green-600'
                      }`}>
                        {machine.current_utilization}%
                      </p>
                      <p className="text-xs text-gray-500">{machine.capacity_hours_per_week}h/week capacity</p>
                    </div>
                  </div>
                );
              })}
      ...
      <Dialog open={showAssignSupplierDialog} onOpenChange={setShowAssignSupplierDialog}>
        <DialogContent className="bg-white max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-gray-900">Assign Supplier to Order</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {selectedOrder && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                <p className="text-sm font-medium text-blue-900">Order: {selectedOrder.id}</p>
                <p className="text-sm text-blue-700">Customer: {selectedOrder.customer_name}</p>
                <p className="text-sm text-blue-700">Value: ${selectedOrder.total_value.toLocaleString()}</p>
              </div>
            )}
            <div className="space-y-3">
