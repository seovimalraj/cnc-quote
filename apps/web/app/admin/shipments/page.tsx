...
export default function AdminShipmentsPage() {
  ...
  const getStatusBadge = (status: Shipment['status']) => {
    const variants = {
      pending: { bg: 'bg-gray-500/10', text: 'text-gray-600', border: 'border-gray-500/20', icon: Clock },
      picked_up: { bg: 'bg-blue-500/10', text: 'text-blue-600', border: 'border-blue-500/20', icon: Package },
      in_transit: { bg: 'bg-yellow-500/10', text: 'text-yellow-600', border: 'border-yellow-500/20', icon: Truck },
      out_for_delivery: { bg: 'bg-purple-500/10', text: 'text-purple-600', border: 'border-purple-500/20', icon: MapPin },
      delivered: { bg: 'bg-green-500/10', text: 'text-green-600', border: 'border-green-500/20', icon: CheckCircle },
      failed: { bg: 'bg-red-500/10', text: 'text-red-600', border: 'border-red-500/20', icon: AlertCircle }
    };
    const variant = variants[status];
    const Icon = variant.icon;
    return (
      <Badge className={`${variant.bg} ${variant.text} ${variant.border} border px-2 py-1 flex items-center gap-1 w-fit`}>
        <Icon size={12} />
  ...
  const handleCreateShipment = () => {
    if (!newShipment.orderId || !newShipment.trackingNumber || !newShipment.customerName || !newShipment.destination) {
    ...
    const shipment: Shipment = {
      ...
      estimatedDelivery: newShipment.estimatedDelivery || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    setShipments(shipments.map(s => 
      s.id === shipmentId 
        ? { 
            ...
            status: newStatus, 
            lastUpdate: new Date().toISOString(),
            actualDelivery: newStatus === 'delivered' ? new Date().toISOString() : s.actualDelivery
          }
        : s
    ));
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-yellow-600 mt-1">{stats.in_transit}</p>
            </div>
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-600 mt-1">{stats.out_for_delivery}</p>
            </div>
          ...
          <TableBody>
            {filteredShipments.map((shipment) => (
              <TableRow key={shipment.id} className="hover:bg-gray-50">
                ...
                <TableCell className="text-gray-600">{shipment.orderId}</TableCell>
                <TableCell className="font-mono text-sm text-gray-600">{shipment.trackingNumber}</TableCell>
                <TableCell>
                  <Badge className="bg-gray-100 text-gray-700 border-0">
                    {shipment.carrier}
                  </Badge>
                </TableCell>
                <TableCell className="text-gray-900">{shipment.customerName}</TableCell>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {shipment.status !== 'delivered' && (
                      <Select 
                        ...
                        onValueChange={(value) => handleUpdateStatus(shipment.id, value as Shipment['status'])}
                      >
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-2">Shipment Info</p>
                  <div className="space-y-2">
                    <p className="text-sm"><span className="font-medium">ID:</span> {selectedShipment.id}</p>
                    <p className="text-sm"><span className="font-medium">Order:</span> {selectedShipment.orderId}</p>
                    <p className="text-sm"><span className="font-medium">Tracking:</span> {selectedShipment.trackingNumber}</p>
                    <p className="text-sm"><span className="font-medium">Carrier:</span> {selectedShipment.carrier}</p>
                  </div>
                </div>
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-2">Package Info</p>
                  <div className="space-y-2">
                    <p className="text-sm"><span className="font-medium">Weight:</span> {selectedShipment.weight} lbs</p>
                    <p className="text-sm"><span className="font-medium">Dimensions:</span> {selectedShipment.dimensions}</p>
                    <p className="text-sm"><span className="font-medium">Ship Date:</span> {new Date(selectedShipment.shipDate).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>

              <div>
                <p className="text-sm font-semibold text-gray-600 mb-2">Delivery Info</p>
                <p className="text-sm mb-1"><span className="font-medium">Customer:</span> {selectedShipment.customerName}</p>
                <p className="text-sm mb-1"><span className="font-medium">Destination:</span> {selectedShipment.destination}</p>
                <p className="text-sm mb-1"><span className="font-medium">Estimated Delivery:</span> {new Date(selectedShipment.estimatedDelivery).toLocaleString()}</p>
                {selectedShipment.actualDelivery && (
                  <p className="text-sm text-green-600 font-medium">Delivered: {new Date(selectedShipment.actualDelivery).toLocaleString()}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-xs text-gray-500">Last updated: {new Date(selectedShipment.lastUpdate).toLocaleString()}</p>
                </div>
              </div>
              ...
              {selectedShipment.notes && (
                <div>
                  ...
                  <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{selectedShipment.notes}</p>
                </div>
              )}

              <div className="flex gap-3 pt-4 border-t">
                <Button onClick={() => handleTrackShipment(selectedShipment)} className="flex-1 bg-blue-600 hover:bg-blue-700">
                  <ExternalLink size={16} className="mr-2" />
                  Track on {selectedShipment.carrier}
                </Button>
                <Button onClick={() => setSelectedShipment(null)} variant="outline" className="flex-1">
                  Close
                </Button>
              </div>
            </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Carrier *</Label>
                  <Select value={newShipment.carrier} onValueChange={(value) => setNewShipment({ ...newShipment, carrier: value as Shipment['carrier'] })}>
