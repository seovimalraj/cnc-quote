...
import { 
  ArrowLeft, Upload, Trash2, Eye, Download, FileText, Package,
  DollarSign, Calendar, User, MessageSquare, Edit
} from 'lucide-react';
import { 
  getOrder, updateOrder,
  getPartsByOrder, createPart, updatePart, deletePart,
  Part, Order
...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleUpdatePart = () => {
    ...
    if (updated) {
      const updatedParts = parts.map(p => p.id === updated.id ? updated : p);
      setParts(updatedParts);
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-3xl font-bold text-white">${order.total_value.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <Badge className={`text-xs ${
                  ...
                  order.payment_status === 'partial' ? 'bg-yellow-500/10 text-yellow-600' :
        ...
        <Card className="bg-white border-gray-200 shadow-sm p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Shipping & Notes</h2>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <Badge className={`text-xs ${
                ...
                order.priority === 'medium' ? 'bg-blue-500/10 text-blue-600' :
                'bg-gray-500/10 text-gray-600'
              }`}>
      ...
      <Card className="bg-white border-gray-200 shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-gray-900">Parts ({parts.length})</h2>
          <Button onClick={() => setShowAddPartDialog(true)} className="bg-blue-600 hover:bg-blue-700">
        ...
        {parts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Add parts to your order to get started</p>
            <Button onClick={() => setShowAddPartDialog(true)} className="bg-blue-600 hover:bg-blue-700">
              <Upload className="w-4 h-4 mr-2" />
              Add First Part
            </Button>
              ...
              <Card key={part.id} className="bg-gray-50 border-gray-200 p-4">
                ...
                <div className="grid grid-cols-4 gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">{part.material}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-600">Quantity</p>
                    <p className="text-sm font-medium text-gray-900">{part.quantity}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-600">Unit Price</p>
                    <p className="text-sm font-medium text-gray-900">${part.unit_price}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-600">Total</p>
                    <p className="text-sm font-bold text-gray-900">${part.total_price.toLocaleString()}</p>
                  </div>
                </div>
      ...
      <Dialog open={showAddPartDialog} onOpenChange={setShowAddPartDialog}>
        <DialogContent className="bg-white max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-gray-900">Add Part to Order</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewPart({ ...newPart, file_name: e.target.value })}
                className="bg-white border-gray-300 text-gray-900 mt-1"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewPart({ ...newPart, tolerance: e.target.value })}
                  className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewPart({ ...newPart, finish: e.target.value })}
                  className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900"
                >
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm font-medium text-blue-900">Total for this part</p>
          ...
          {editingPart && (
            <div className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditingPart({ ...editingPart, tolerance: e.target.value })}
                    className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900"
                  >
                    {tolerances.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm font-medium text-blue-900">Total for this part</p>
                <p className="text-2xl font-bold text-blue-600">
                  ${(editingPart.unit_price * editingPart.quantity).toLocaleString()}
                </p>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowEditPartDialog(false)}>
              Cancel
            </Button>
            <Button onClick={handleUpdatePart} className="bg-blue-600 hover:bg-blue-700">
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
