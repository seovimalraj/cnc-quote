...
import { 
  TrendingUp, TrendingDown, DollarSign, Clock, Star, Package, Factory, 
  CheckCircle, AlertCircle, Target, Users, Calendar, Award, FileCheck,
  BarChart3, Download
} from 'lucide-react';
...
export default function SupplierAnalyticsPage() {
  ...
  // Comprehensive KPIs
  const kpis = [
    { label: 'Total Revenue', value: '$284,500', change: '+18.5%', trend: 'up', icon: DollarSign, color: 'green', subtext: 'Last 30 days' },
    { label: 'Orders Completed', value: '156', change: '+12', trend: 'up', icon: Package, color: 'blue', subtext: '24 in progress' },
    { label: 'Avg Order Value', value: '$1,824', change: '+$142', trend: 'up', icon: BarChart3, color: 'purple', subtext: 'Per order' },
    { label: 'On-Time Delivery', value: '94.2%', change: '+2.4%', trend: 'up', icon: CheckCircle, color: 'emerald', subtext: '147 of 156' },
    { label: 'Quality Score', value: '96.8%', change: '+1.2%', trend: 'up', icon: Award, color: 'yellow', subtext: 'First pass rate' },
    { label: 'Customer Rating', value: '4.8/5', change: '+0.2', trend: 'up', icon: Star, color: 'orange', subtext: '42 reviews' },
    { label: 'Avg Lead Time', value: '6.2 days', change: '-1.3 days', trend: 'down', icon: Clock, color: 'cyan', subtext: 'Improved' },
    { label: 'Capacity Used', value: '78%', change: '+5%', trend: 'up', icon: Factory, color: 'indigo', subtext: '22% available' },
  ];
  ...
  const topCustomers = [
    { name: 'AeroSpace Dynamics', orders: 42, revenue: 89500, growth: '+18%', rating: 4.9 },
    { name: 'MedDevice Inc', orders: 38, revenue: 76200, growth: '+22%', rating: 4.8 },
    { name: 'Acme Manufacturing', orders: 35, revenue: 58900, growth: '+12%', rating: 4.7 },
    { name: 'TechCorp Industries', orders: 28, revenue: 42800, growth: '+8%', rating: 4.6 },
    { name: 'Global Robotics Ltd', orders: 13, revenue: 17100, growth: '+5%', rating: 4.5 },
  ];
  ...
  const performanceMetrics = [
    { metric: 'On-Time Delivery', current: 94.2, target: 95, previous: 91.8, trend: 'up' },
    { metric: 'Quality First Pass', current: 96.8, target: 98, previous: 95.6, trend: 'up' },
    { metric: 'Customer Satisfaction', current: 4.8, target: 4.9, previous: 4.6, trend: 'up' },
    { metric: 'Response Time (hrs)', current: 2.4, target: 2.0, previous: 3.1, trend: 'down' },
  ];

  const getChartData = () => {
    if (chartView === 'revenue') return revenueData.map(d => ({ ...d, value: d.revenue, max: maxRevenue }));
    if (chartView === 'orders') return revenueData.map(d => ({ ...d, value: d.orders, max: maxOrders }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((kpi) => {
          ...
          return (
            <Card key={kpi.label} className="hover:shadow-lg transition-shadow bg-white">
      ...
      <Card className="hover:shadow-md transition-shadow bg-white">
        ...
        <CardContent>
          <div className="space-y-3">
            {getChartData().filter(d => d.value > 0).map((data) => (
              <div key={data.month} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 font-medium w-12">{data.month}</span>
                  <span className="text-gray-900 font-semibold">{formatChartValue(data.value)}</span>
                </div>
                ...
                <div key={customer.name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm font-bold text-gray-900">${(customer.revenue / 1000).toFixed(1)}K</span>
                    </div>
                ...
                <div key={metric.metric} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-600">Target: {metric.target}{metric.metric.includes('%') ? '%' : ''}</span>
                      {metric.trend === 'up' ? (
                  ...
                  <div className="relative h-6 bg-gray-100 rounded-full overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-xs font-semibold text-gray-900">{metric.current}{metric.metric.includes('%') ? '%' : ''}</span>
                    </div>
                  </div>
                  <p className="text-xs text-gray-500">Previous: {metric.previous}{metric.metric.includes('%') ? '%' : ''}</p>
                </div>
              ))}
            </div>
          </CardContent>
              ...
              <div key={item.material} className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="outline" className="text-xs">{item.orders} orders</Badge>
                  </div>
