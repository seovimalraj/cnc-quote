...
import { getSupplierDashboardStats, getSupplierActiveOrders } from '@/lib/database';
...
export default function SupplierDashboardPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      change: stats.machineUtilization > 80 ? 'High capacity' : 'Capacity available',
  ...
  const alerts = [
    { type: 'warning', message: 'Low material stock: Aluminum 6061 (85kg remaining)', action: 'View Inventory', href: '/supplier/inventory' },
    { type: 'warning', message: 'CNC Mill #3 at 98% utilization', action: 'View Capacity', href: '/supplier/capacity' },
    { type: 'success', message: '5 orders completed this week', action: 'View Orders', href: '/supplier/orders' },
    { type: 'info', message: '3 new RFQs awaiting response', action: 'View RFQs', href: '/supplier/orders' },
  ];
  ...
  const getAlertColor = (type: string) => {
    if (type === 'warning') return 'bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800 text-yellow-900 dark:text-yellow-100';
    if (type === 'success') return 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800 text-green-900 dark:text-green-100';
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-3xl font-bold text-gray-900 dark:text-white">
                    {loading ? <Loader2 className="w-8 h-8 animate-spin inline" /> : stat.value}
                  </p>
                  <p className="text-sm text-green-600 dark:text-green-400 flex items-center gap-1">
                    {!loading && stat.trend === 'up' && <TrendingUp size={14} />}
                    {stat.change}
                  </p>
                </div>
              </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <AlertIcon className={`flex-shrink-0 mt-0.5 ${alert.type === 'warning' ? 'text-yellow-600' : alert.type === 'success' ? 'text-green-600' : 'text-blue-600'}`} size={20} />
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {order.customer_company || order.customer_name} • {Array.isArray(order.parts) ? order.parts.length : 0} parts
                    </p>
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl p-8 shadow-2xl shadow-emerald-500/30">
        <div className="flex items-center justify-between">
          <div className="text-white">
            ...
            <p className="text-emerald-100">Manage machine capacity and schedule efficiently</p>
          </div>
          <Link
