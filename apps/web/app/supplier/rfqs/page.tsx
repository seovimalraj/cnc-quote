...
import { 
  FileText, Clock, DollarSign, Package, Search, 
  Eye, AlertCircle, TrendingUp, Filter, Loader2
} from 'lucide-react';
...
export default function SupplierRFQsPage() {
  ...
  const bidSubmittedCount = rfqs.filter(r => r.status === 'bid-submitted').length;
  const totalValue = rfqs.filter(r => r.status === 'open').reduce((sum, r) => sum + r.display_value, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Review and bid on manufacturing opportunities</p>
        </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-green-600 mt-1">Available for bidding</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{bidSubmittedCount}</p>
          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant={filterStatus === 'bid-submitted' ? 'default' : 'outline'}
                  onClick={() => setFilterStatus('bid-submitted')}
                  className={filterStatus === 'bid-submitted' ? 'bg-blue-600' : ''}
                >
                  Submitted
                </Button>
              </div>
        ...
        <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-yellow-900">Privacy Protected RFQs</p>
            <p className="text-sm text-yellow-800">
              Customer information is masked for privacy. Click "View Details" to unlock full specifications and submit your bid.
            </p>
          </div>
        </div>
          ...
          {filteredRFQs.length === 0 ? (
            <Card>
              <CardContent className="p-12 text-center">
                ...
                {rfqs.length === 0 ? (
                  <>
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">No RFQs Available</h3>
                    <p className="text-gray-600 mb-4">
                      There are currently no open RFQs to bid on.
                    </p>
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md mx-auto">
                      <p className="text-sm text-blue-900">
                        RFQs will appear here when customers place orders that require manufacturing quotes.
                  ...
                  <>
                    <p className="text-gray-600">No RFQs found matching your criteria</p>
                    <Button
                      variant="outline"
                      className="mt-4"
                      onClick={() => {
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">{rfq.id}</h3>
                        <Badge className={
                          rfq.status === 'open' ? 'bg-green-100 text-green-700 border-0' :
                          rfq.status === 'bid-submitted' ? 'bg-blue-100 text-blue-700 border-0' :
                          'bg-gray-100 text-gray-700 border-0'
                        }>
                          {rfq.status === 'open' ? 'Open for Bidding' :
                           rfq.status === 'bid-submitted' ? 'Bid Submitted' :
                           'Closed'}
                        </Badge>
                        <span className="text-sm text-gray-500">{getTimeAgo(rfq.created_at)}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="font-semibold text-gray-900">{rfq.parts}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="font-semibold text-gray-900">{rfq.lead_time} days</p>
                          </div>
                        </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-600">
                        <strong className="text-gray-900">Limited Preview:</strong> This is basic information only. 
                        View full specifications, 3D models, and technical drawings to submit your bid.
                      </div>
                    </div>
                    ...
                    <div className="ml-6">
                      <Button
                        ...
                        className={rfq.status === 'open' ? 'bg-blue-600 hover:bg-blue-700' : ''}
                      >
                        <Eye className="w-4 h-4 mr-2" />
                        {rfq.status === 'open' ? 'View & Bid' : 
                         rfq.status === 'bid-submitted' ? 'View Bid' : 
                         'View Details'}
                      </Button>
                    </div>
                  </div>
