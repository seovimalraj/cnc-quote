...
export default function SupplierCertificationsPage() {
  const [certifications, setCertifications] = useState<SupplierCertification[]>([
    {
      ...
      verifiedAt: new Date(Date.now() - 300 * 24 * 60 * 60 * 1000).toISOString()
    },
    {
      id: 'CERT-002',
      certType: 'AS9100D',
      ...
      expiryDate: new Date(Date.now() + 900 * 24 * 60 * 60 * 1000).toISOString(),
      status: 'active',
      verificationStatus: 'verified',
      documentUrl: '/documents/as9100-cert.pdf',
      verifiedBy: 'Admin Team',
      verifiedAt: new Date(Date.now() - 150 * 24 * 60 * 60 * 1000).toISOString()
    },
    {
      id: 'CERT-003',
      ...
      expiryDate: new Date(Date.now() + 1035 * 24 * 60 * 60 * 1000).toISOString(),
      status: 'active',
    ...
    },
    {
      ...
      verifiedBy: 'Admin Team',
      verifiedAt: new Date(Date.now() - 290 * 24 * 60 * 60 * 1000).toISOString()
    }
  ]);

  const [selectedCert, setSelectedCert] = useState<SupplierCertification | null>(null);
  ...
  const getStatusBadge = (status: SupplierCertification['status']) => {
    const variants = {
      ...
      expiring_soon: { bg: 'bg-orange-100', text: 'text-orange-700', icon: AlertTriangle, label: 'Expiring Soon' }
    };
  ...
  const getVerificationBadge = (status: SupplierCertification['verificationStatus']) => {
    const variants = {
      verified: { bg: 'bg-green-100', text: 'text-green-700', icon: Shield, label: 'Verified' },
  ...
  const handleUploadCertification = () => {
    if (!newCert.certType || !newCert.certNumber || !newCert.issuingBody || !newCert.issueDate || !newCert.expiryDate) {
    ...
    alert(`✅ Certification uploaded successfully! It will be reviewed by the admin team.`);
  };
  ...
  const handleDeleteCertification = (certId: string) => {
    ...
    if (confirm(`Are you sure you want to delete certification "${cert?.certType}"?`)) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your certifications and compliance documents</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {certifications.map((cert) => {
          ...
          return (
            <Card key={cert.id} className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <Award size={20} className="text-emerald-600" />
                      <h3 className="text-lg font-semibold text-gray-900">{cert.certType}</h3>
                    </div>
                    <p className="text-sm font-mono text-gray-600 mb-2">{cert.certNumber}</p>
                  ...
                  <div className="flex gap-2">
                    ...
                    {cert.documentUrl && (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => alert(`Downloading: ${cert.documentUrl}`)}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className={`font-medium ${daysUntilExpiry < 90 ? 'text-orange-600' : 'text-gray-900'}`}>
                  ...
                  {cert.verificationStatus === 'verified' && cert.verifiedBy && (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <p className="text-sm"><span className="font-medium">Certificate Number:</span> {selectedCert.certNumber}</p>
                  <p className="text-sm"><span className="font-medium">Issuing Body:</span> {selectedCert.issuingBody}</p>
                  <p className="text-sm"><span className="font-medium">Issue Date:</span> {new Date(selectedCert.issueDate).toLocaleDateString()}</p>
                  <p className="text-sm"><span className="font-medium">Expiry Date:</span> {new Date(selectedCert.expiryDate).toLocaleDateString()}</p>
                  <p className="text-sm"><span className="font-medium">Days Until Expiry:</span> {getDaysUntilExpiry(selectedCert.expiryDate)} days</p>
                </div>
              </div>

              {selectedCert.verificationStatus === 'verified' && selectedCert.verifiedBy && (
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-2">Verification Details</p>
                  <div className="space-y-2">
                    <p className="text-sm"><span className="font-medium">Verified By:</span> {selectedCert.verifiedBy}</p>
                    <p className="text-sm"><span className="font-medium">Verification Date:</span> {new Date(selectedCert.verifiedAt!).toLocaleDateString()}</p>
                  </div>
                </div>
              )}
              ...
              {selectedCert.notes && (
                <div>
                  ...
                  <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{selectedCert.notes}</p>
                </div>
              )}
              ...
              {selectedCert.documentUrl && (
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-3 rounded">
                    ...
                    <span className="text-sm font-mono flex-1">{selectedCert.documentUrl}</span>
                    <Button size="sm" onClick={() => alert(`Downloading: ${selectedCert.documentUrl}`)}>
              ...
              <div className="flex gap-3 pt-4 border-t">
                <Button onClick={() => setEditingCert({ ...selectedCert })} className="flex-1 bg-emerald-600 hover:bg-emerald-700">
                  <Edit size={16} className="mr-2" />
                  Edit
                </Button>
                {selectedCert.documentUrl && (
                  <Button onClick={() => alert(`Downloading: ${selectedCert.documentUrl}`)} variant="outline" className="flex-1">
      ...
      {(showUploadDialog || editingCert) && (
        <Dialog open={showUploadDialog || !!editingCert} onOpenChange={() => {
          setShowUploadDialog(false);
          setEditingCert(null);
          setUploadedFile(null);
        }}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="ITAR Registration">ITAR Registration</SelectItem>
                    <SelectItem value="FDA Registered">FDA Registered</SelectItem>
                    <SelectItem value="RoHS Compliant">RoHS Compliant</SelectItem>
              ...
              <div>
                ...
                <Input
                  value={editingCert ? editingCert.certNumber : newCert.certNumber || ''}
                  onChange={(e) => editingCert 
                    ? setEditingCert({ ...editingCert, certNumber: e.target.value })
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    type="date"
                    value={editingCert ? editingCert.issueDate.split('T')[0] : newCert.issueDate?.split('T')[0] || ''}
                    onChange={(e) => editingCert 
                      ? setEditingCert({ ...editingCert, issueDate: new Date(e.target.value).toISOString() })
                      : setNewCert({ ...newCert, issueDate: new Date(e.target.value).toISOString() })
                    }
                    className="bg-white border-gray-300"
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="date"
                    value={editingCert ? editingCert.expiryDate.split('T')[0] : newCert.expiryDate?.split('T')[0] || ''}
                    onChange={(e) => editingCert 
                      ? setEditingCert({ ...editingCert, expiryDate: new Date(e.target.value).toISOString() })
                      : setNewCert({ ...newCert, expiryDate: new Date(e.target.value).toISOString() })
                    }
                    className="bg-white border-gray-300"
                  />
                </div>
              ...
              <div>
                <Label>Notes</Label>
                <Textarea
                  value={editingCert ? editingCert.notes || '' : newCert.notes || ''}
                  onChange={(e) => editingCert
                    ? setEditingCert({ ...editingCert, notes: e.target.value })
            ...
            <DialogFooter>
              {editingCert ? (
                <>
                  <Button onClick={handleUpdateCertification} className="bg-emerald-600 hover:bg-emerald-700">
                ...
                <>
                  <Button onClick={handleUploadCertification} className="bg-emerald-600 hover:bg-emerald-700">
